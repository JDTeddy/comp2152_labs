...
def get_events_by_severity(severity):
    with sqlite3.connect(DB_NAME) as conn:
        return conn.execute("SELECT * FROM audit_log WHERE severity = ?", (severity,)
                            ).fetchall()


def get_recent_events(limit):
    with sqlite3.connect(DB_NAME) as conn:
        return conn.execute("SELECT * FROM audit_log ORDER BY timestamp DESC LIMIT ?", (limit,)
                            ).fetchall()


def count_by_severity():
    with sqlite3.connect(DB_NAME) as conn:
        return conn.execute("SELECT severity, COUNT(*) FROM audit_log GROUP BY severity ORDER BY COUNT(*) DESC"
                            ).fetchall()
